...
cifar10_query_dataset = Subset(cifar10_dataset, indices=[x for x in range(100,200)])
cifar10_private_dataset = Subset(cifar10_dataset, indices=[x for x in range(200,300)])
cifar10_unlabeled_dataset = Subset(MyUnlabeledCIFAR10(cifar10_dataset), indices=[x for x in range(300,4000)])

args = {'batch_size': 20, 'device':'cuda', 'num_partitions':1, 'wrapped_strategy_class': None,'unlabeled':True, 'embedding_type':'features', 'smi_function':'fl2mi', 'scg_function':'flcg', "scmi_function":"flcmi"}
if(args['embedding_type']=="gradients"):
    cifar10_model = ResNet18(10).to("cuda")
if(args['embedding_type']=="features"):
    cifar10_model =  models.resnet18().to("cuda")
...
if(test=="scg"):
    ...
    print(args)
    my_cifar10_strategy = SMI(cifar10_labeled_dataset, cifar10_unlabeled_dataset, cifar10_query_dataset, cifar10_model, 10, args)
...
part_indices = my_part_cifar10_strategy.select(100)
print(part_indices)
print(len(part_indices))
print("Number of common elements in partition strategy: ", set(indices).intersection(set(part_indices)))